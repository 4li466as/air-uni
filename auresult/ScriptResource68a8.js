...
Sys.Net.WebRequest._resolveUrl = function Sys$Net$WebRequest$_resolveUrl(url, baseUrl) {
    ...
    baseUrl = baseUrl.substr(0, baseUrl.lastIndexOf('https://portals.au.edu.pk/') + 1);
    if (!url || url.length === 0) {
        return baseUrl;
    }
    if (url.charAt(0) === 'https://portals.au.edu.pk/') {
        var slashslash = baseUrl.indexOf('://');
        if (slashslash === -1) {
            throw Error.argument("baseUrl", Sys.Res.badBaseUrl1);
        }
        var nextSlash = baseUrl.indexOf('https://portals.au.edu.pk/', slashslash + 3);
        if (nextSlash === -1) {
        ...
        var lastSlash = baseUrl.lastIndexOf('https://portals.au.edu.pk/');
        if (lastSlash === -1) {
